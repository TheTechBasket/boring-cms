...
- **Schema model**: custom collections builder. Collections and fields defined per project from the UI. Field types live in one registry (`lib/content.ts` FIELD_TYPE_DEFS): text, markdown, number, boolean, date, datetime, json, image, relation, counter, countermap. `GET /api/v1/<project>/field-types` reports the live list.
- **Counters**: `counter` (up/down per entry) and `countermap` (named keys per entry, `group:option` keys switch a visitor's vote inside the group, `maxKeys` cap) never touch entry data, ETags or webhooks. Totals live in memory and flush to the per-project `counters` table every 5 seconds; countermap keys share that table with the field stored as `field + "\x1f" + key`, so no migration was needed. Voter dedupe is a salted in-memory hash with 24 hour TTL, forgotten on restart.
